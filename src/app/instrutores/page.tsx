...
export default function InstrutoresPage() {
  ...
  const canSave = useMemo(() => {
    if (editing) {
      return !!(form.nome && form.cpf && form.email && (form.status === 'Ativo' || form.status === 'Inativo'));
    }
    return !!(form.nome && form.cpf && form.email && form.senha && form.status);
  }, [form, editing]);

  async function fetchProfs(query?: string) {
    try {
      setLoading(true);
      const url = query?.trim() ? `/api/professores?q=${encodeURIComponent(query)}` : '/api/professores';
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    const res = await fetch('/api/professores', {
      ...
      body: JSON.stringify(
        editing
          ? { id_professor: editing.id_professor, ...form }
          : { ...form }
      ),
    });
  ...
  async function handleEdit(p: Professor) {
    setEditing(p);
    setForm({ ...p, senha: '' }); // não mostra senha atual
  }
        ...
        <section className="section">
          <div className="flex flex-col md:flex-row gap-3 md:items-center md:justify-between">
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={() => { setQ(''); fetchProfs(); }}
                className="px-3 py-2 rounded-lg border hover:bg-gray-50 text-sm"
              >
                Limpar
              </button>
            </div>
            <div className="text-sm muted">{Array.isArray(list) ? list.length : 0} resultado(s)</div>
          </div>
        </section>

        <section className="mt-6 grid gap-6 lg:grid-cols-3">
          {/* Form */}
          <form onSubmit={handleCreate} className="section space-y-4 lg:col-span-1">
            <h2 className="h2">{editing ? 'Editar Instrutor' : 'Novo Instrutor'}</h2>
                  ...
                  {loading ? (
                    [...Array(5)].map((_, i) => (
                      <tr key={i}>
                        ...
                        <td className="py-2 pr-4"><div className="h-4 w-20 bg-gray-200 rounded animate-pulse" /></td>
                        <td className="py-2"><div className="h-4 w-24 bg-gray-200 rounded animate-pulse" /></td>
                      </tr>
                    ))
                  ) : Array.isArray(list) && list.length > 0 ? (
                    list.map((p) => (
                      <tr key={p.id_professor} className="border-b hover:bg-gray-50">
                    ...
                    <tr><td className="py-4 text-gray-500" colSpan={5}>Nenhum instrutor encontrado.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
